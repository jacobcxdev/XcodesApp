...
3. Commit the version change. Create an annotated tag whose values exactly match the project, for example:
   ...
   ```sh
   git tag -a v4.1.1b49 -m 'Xcodes 4.1.1 build 49'
   git push origin v4.1.1b49
   ```
...
For a manual rerun, `workflow_dispatch` reruns must use `--ref v4.1.1b49` and the same `release_tag`; selecting a branch is rejected before credential files are written:

```sh
gh workflow run release.yml --ref v4.1.1b49 -f release_tag=v4.1.1b49
```

A manual appcast rerun must likewise use the published tag for both the workflow ref and input (`--ref <tag> -f tag=<same tag>`):

```sh
gh workflow run appcast.yml --ref v4.1.1b49 -f tag=v4.1.1b49
```
...
bash Scripts/package_release.sh v4.1.1b49
bash Scripts/validate_release_artifacts.sh Product/v4.1.1b49 v4.1.1b49
```
