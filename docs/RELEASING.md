...
3. Commit the version change. Create an annotated tag whose values exactly match the project, for example:
   ...
   ```sh
   git tag -a v4.0.4b39 -m 'Xcodes 4.0.4 build 39'
   git push origin v4.0.4b39
   ```
...
bash Scripts/package_release.sh v4.0.4b39
bash Scripts/validate_release_artifacts.sh Product/v4.0.4b39 v4.0.4b39
```
...
- Appcast failure after a valid release: Rerun the same appcast tag only for transient infrastructure or Pages configuration failures when the tagged source is unchanged. Do not replace signed release assets under an existing tag. Any workflow, validator, or source fix requires an incremented build number, a new immutable `vX.Y.ZbN` tag, and a new release.
